...
- **Cx. Fin. Cobros** (`/vtas-cobros/cx-fin-cobros`, alias `/finanzas/analisis-mc/costos-financieros`): sidenav **VTAS. & COBROS → Cx. Fin. Cobros**. Header **VTAS. & COBROS** / Cx. Fin. Cobros. Matriz **FORMA DE PAGO × ENTIDAD [× CUOTA]** (`fin_ana_cos_fina`: `pago_id` → `cobros_forma_pago`, `terminal_id` → `tesoreria_cobros_entidades`, `cuota_id` → `cobros_cuotas` si `acepta_cuotas`). Columnas: **HAB.** | **FORMA DE PAGO** | **ENTIDAD** | **CUOTAS** (etiqueta `cobros_cuotas.cuotas` o `—`) | **DÍAS DE ACREDITACIÓN** | **ARANCEL** | **CX FINANCIERO** | **IMP. CHEQUE** | **CX TOTAL S/ IVA** | **CX TOTAL C/ IVA**. Orden alfabético de filas: **FORMA DE PAGO** → **ENTIDAD** → **CUOTAS**. **GESTIONAR ENTIDADES** / **GESTIONAR FORMAS PAGO** / **GESTIONAR CUOTAS** / **Cálculo Cx. Total** (menú ACCIONES). Formas Pago: mismo patrón de catálogo que Entidades (§1.4); **sin** arrastre/orden (Margen Contribución ordena columnas alfabéticamente). Alta/edición: checkboxes de **entidades** (N:M `cobros_forma_pago_entidades`, mín. 1) + switch **Acepta cuotas** (`acepta_cuotas`); la lista muestra las entidades asociadas. **Gestionar Cuotas:** texto libre MAYÚSCULAS (ej. `01`, `03`, `06 PROMOCION`); sin columna `orden`.
