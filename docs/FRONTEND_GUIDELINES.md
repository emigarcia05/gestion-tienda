...
- **Cx. Fin. Cobros** (`/vtas-cobros/cx-fin-cobros`, alias `/finanzas/analisis-mc/costos-financieros`): sidenav **VTAS. & COBROS → Cx. Fin. Cobros**. Header **VTAS. & COBROS** / Cx. Fin. Cobros. Matriz **MARCA × PAGO** (`fin_ana_cos_fina.terminal_id` → `fin_ana_cos_fina_terminales_marcas`). **Gestionar Marcas** (nombre; alta/edición en modal aparte). **Gestionar Terminales**: catálogo `fin_ana_cos_fina_terminales` (ID DUX + marca + titular `global_pto_vtas.nombre_titular`; alta/edición en modal aparte). Pagos y **Cálculo Cx. Total**.
