...
- **Encabezado de columnas (opcional):** `TYPEAHEAD_LISTBOX_HEADER_CLASS` (altura `--tabla-thead-height`, `text-xs`) + grid propio del módulo. Celdas `w-full text-center`. Header y `ul` con `[scrollbar-gutter:stable]` para que las columnas no se desfasen con la barra de scroll.
