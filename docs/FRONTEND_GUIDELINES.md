...
**Administración** (`AdministracionAccordionNav`): **FINANZAS** (TESORERIA → Fondos / Flujo De Fondos | BALANCE | OPERACIONES → COMPRAS / GASTOS | IMPUESTOS) → **LISTA PRECIOS** (PX TIENDA | PROVEEDORES | ANÁLISIS M.C.) → **VTAS. & COBROS** (Ptos. Vtas. / Cx. Fin. Cobros / Cobros por sucursal, pantallas directas) → **PEDIDO A FÁB.** → **ESTADÍSTICAS** → **USUARIOS**. Acordeón anidado: el grupo padre sigue abierto mientras un subgrupo hijo está expandido. **IMPUESTOS** agrupa Posición De IVA (`/finanzas/posicion-iva`).
...
| Cobros por sucursal | `/vtas-cobros/cobros-por-sucursal` |
...
- **Cx. Fin. Cobros** (`/vtas-cobros/cx-fin-cobros`, alias `/finanzas/analisis-mc/costos-financieros`): sidenav **VTAS. & COBROS → Cx. Fin. Cobros**. Header **VTAS. & COBROS** / Cx. Fin. Cobros. Filtros en `FilaFiltrosDesplegables` (5 columnas, orden fijo): **HABILITADO** (`si`/`no` → HABILITADO / NO HABILITADO) · **FORMA DE PAGO** · **ENTIDAD** · **CUOTAS** (`cobros_cuotas.id` vs `cuota_id`) · **IMP. CHEQUE** (`si`/`no` → SI / NO). Contador **COMBINACIÓN(ES)** + `LimpiarFiltrosButton` en una segunda fila (no `col-span-2` dentro del grid). Matriz **FORMA DE PAGO × ENTIDAD [× CUOTA]** (`cobros_cx_fin` ex `fin_ana_cos_fina`: `pago_id` → `cobros_forma_pago`, `terminal_id` → `tesoreria_cobros_entidades`, `cuota_id` → `cobros_cuotas` si `acepta_cuotas`). Columnas: **HAB.** | **FORMA DE PAGO** | **ENTIDAD** | **CUOTAS** (etiqueta `cobros_cuotas.cuotas` o `—`) | **DÍAS DE ACREDITACIÓN** | **ARANCEL** | **CX FINANCIERO** | **IMP. CHEQUE** | **CX TOTAL S/ IVA** | **CX TOTAL C/ IVA**. Orden alfabético de filas: **FORMA DE PAGO** → **ENTIDAD** → **CUOTAS**. **GESTIONAR ENTIDADES** / **GESTIONAR FORMAS PAGO** / **GESTIONAR CUOTAS** / **Cálculo Cx. Total** (menú ACCIONES). Formas Pago: mismo patrón de catálogo que Entidades (§1.4); **sin** arrastre/orden (Margen Contribución ordena columnas alfabéticamente). Alta/edición: checkboxes de **entidades** (N:M `cobros_forma_pago_entidades`, mín. 1) + switch **ACEPTA CUOTAS** (`ModalSiNoChoice`, `acepta_cuotas`); la lista muestra las entidades asociadas. **Gestionar Cuotas:** texto libre MAYÚSCULAS (ej. `01`, `03`, `06 PROMOCION`); sin columna `orden`.
- **Cobros por sucursal** (`/vtas-cobros/cobros-por-sucursal`): sidenav **VTAS. & COBROS → Cobros por sucursal**. Filas = combinaciones **forma de pago × entidad** con al menos una fila `habilitado` en `cobros_cx_fin` (todas las cuotas de esa combinación comparten destino). Columnas = sucursales con depósito. Cada celda = Select de caja destino (misma entidad; sin CHEQUE; `none` = SIN DESTINO). Tabla `cobros_por_sucursal` UNIQUE `(pago_id, entidad_id, sucursal_id)`; Suc1→Caja1 y Suc2→Caja2 (o misma caja). Editor guarda al cambiar el Select.
...
- [ ] Labels de modal en `text-foreground`. Boolean / SÍ/NO de modal: `ModalSiNoChoice` (MAYÚSCULAS, sin texto de ayuda).
