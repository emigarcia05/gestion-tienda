...
- **Cx. Fin. Cobros** (`/vtas-cobros/cx-fin-cobros`, alias `/finanzas/analisis-mc/costos-financieros`): sidenav **VTAS. & COBROS → Cx. Fin. Cobros**. Header **VTAS. & COBROS** / Cx. Fin. Cobros. Matriz **MARCA × PAGO** (`fin_ana_cos_fina.terminal_id` → `cobros_cx_fin_marcas`). **Gestionar Marcas** (nombre; alta/edición en modal aparte). **Gestionar Pagos** (`cobros_forma_pago`; `ModalSiNoChoice` **ASOCIADO TERMINAL** / **ASOCIADO BANCO**). **Gestionar Bancos** (`cobros_bancos`; mismo botón que Pagos/`Settings2`; modal `GestionarBancosCobrosModal` con búsqueda + CRUD nombre MAYÚSCULAS). **Cálculo Cx. Total**.
