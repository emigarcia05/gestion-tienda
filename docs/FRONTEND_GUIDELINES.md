...
- **Lista Precios** (`/proveedores/lista-precios`): botón **Edición Masiva** (`EdicionMasivaListaPreciosModal` modo header): PROVEEDOR obligatorio, MARCA opcional, RUBRO opcional (Select habilitado solo con MARCA), VARIACIÓN ± % 2 dec. obligatoria (`PorcentajeCentInput` `allowNegative`, sufijo `%` fijo no editable). Confirmar aplica `px_lista_proveedor * (1 + variación/100)` (piso 0; p. ej. $100 y 4% → $104; −4% → $96). No exige filtro de página. El lápiz de fila es SET de marca/rubro/px lista. **ACCIONES**: Descuentos (%) → Editar → Vincular REX → Borrar. **Descuentos Aplicados** (ícono %): `AppModal` `size="sm"` + `max-w-[33.6rem]` (20 % más ancho que `sm`). Grilla etiqueta 40 % | $ 30 % | % 20 % (valores $ y % centrados). **PX. PROMO FIJO** (moneda del ítem; tacho a la derecha del monto borra el promo y vuelve a las reglas). Debajo, tabla compacta (cebra) **PX. LISTA** ± reglas (descuento `−$`, Cx. `+$`; o `-` si hay promo; % + info) y **PX. FINAL** en `TableFooter` (`bg-muted`, borde `primary`).
