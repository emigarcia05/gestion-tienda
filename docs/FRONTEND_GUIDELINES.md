...
SSOT `FACTURACION_ROUTES` (`src/lib/facturacionRoutes.ts`). Módulo sidenav **FACTURA**: Crear / Facturas / Presupuestos. **Crear** (`FacturaCrearPageClient`): cabecera en card — **una fila** `grid-cols-4` (Fecha, Tipo De Factura MAYÚSCULAS, N° Comprobante, Cliente). Segundo bloque (`FacturaCrearLineasBlock`): lupa (avanzada stub) + input; la **lista fija** (`h-72`) solo se abre con **≥ 3 letras** (COD. / DESCRIPCIÓN / PRECIOS / STOCK / ícono sucursal `Store`); stock local `0` → `AlertTriangle` `text-destructive`; si no hay stock local pero sí en otra sucursal, `Store` en `text-primary`; click fila = agregar (cierra + foco CANTIDAD con select; **Enter** en cantidad vuelve al buscador); ícono sucursal abre `FacturaProductoStockModal` (descripción + sucursal/stock). Stock de columna = sucursal del usuario; detalle = sucursales con depósito. Tabla remito en `.contenedor-tabla-gestion` (scroll vertical; **thead sticky**; al agregar ítem nuevo scroll al final): basura | COD. | DESCRIPCIÓN | CANTIDAD | PX. LISTA | TOTAL. Tipos: `FACTURA_TIPOS` / `FACTURA_TIPO_LABELS` en `@/lib/factura`. Facturas / Presupuestos: placeholders CFTL sin datos.
