...
- **Urgente** (`PedidoUrgentePageClient`): sucursal (preferida) basta para listar. Catálogo: `prod_precios_provee.habilitado` de mercadería no fábrica. 7 columnas (PROVEEDOR 10%, DESCRIPCIÓN 60%, CANT. PED. / PROV. PED. / cesto / CONF. REPO. / CANT. REPO. 6% c/u). Sin columna VINC.; primero **Productos Registrados en Dux** (`prod_tienda`) y después **Sin Registrar** (`TablaSubencabezadoSeccionRow`). Filtro PEDIDO: urgente / reposición / cualquiera. Doble clic: varios proveedores → Elegir Proveedor (también con filtro PROVEEDOR). Header: solo `GenerarPedidoToolbarButton`. Auto-refresh IVA: `PosicionIvaComparacionAutoRefresh`.
