...
- **Cx. Fin. Cobros** (`/vtas-cobros/cx-fin-cobros`, alias `/finanzas/analisis-mc/costos-financieros`): sidenav **VTAS. & COBROS → Cx. Fin. Cobros**. Header **VTAS. & COBROS** / Cx. Fin. Cobros. Matriz **FORMA DE PAGO × ENTIDAD** (`fin_ana_cos_fina`: `pago_id` → `cobros_forma_pago`, `terminal_id` → `cobros_entidades`). Columnas: **HAB.** | **FORMA DE PAGO** | **ENTIDAD** | **CUOTAS** (catálogo `cobros_cuotas`; UI placeholder hasta cablear FK) | **DÍAS DE ACREDITACIÓN** | **ARANCEL** | **CX FINANCIERO** | **IMP. CHEQUE** | **CX TOTAL S/ IVA** | **CX TOTAL C/ IVA**. **Gestionar Entidades**. **Gestionar Formas Pago** (`cobros_forma_pago`). **Gestionar Cuotas** (`cobros_cuotas`; CRUD cantidad 1–99). **Cálculo Cx. Total**.
