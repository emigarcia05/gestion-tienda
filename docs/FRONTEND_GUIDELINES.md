...
SSOT `FACTURACION_ROUTES` (`src/lib/facturacionRoutes.ts`). Módulo sidenav **FACTURA**: Crear / Facturas / Presupuestos. **Crear** (`FacturaCrearPageClient`): cabecera en card — grid 3 cols (Fecha con picker nativo AR, Tipo De Factura select con labels MAYÚSCULAS, N° Comprobante solo lectura vacío) + Cliente en 2.ª fila. Segundo bloque (`FacturaCrearLineasBlock`): lupa (avanzada stub) + input; al foco/click abre **lista fija** (`h-72`) con columnas COD. / DESCRIPCIÓN / PRECIOS / STOCK / lupa; filtrado desde **3 letras** (máx. 10); click fila = agregar (cierra + foco CANTIDAD con select); lupa de fila abre `FacturaProductoStockModal` (descripción + sucursal/stock). Stock de columna = sucursal del usuario; detalle = sucursales con depósito. Tipos: `FACTURA_TIPOS` / `FACTURA_TIPO_LABELS` en `@/lib/factura`. Facturas / Presupuestos: placeholders CFTL sin datos.
