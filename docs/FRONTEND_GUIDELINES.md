...
SSOT `FACTURACION_ROUTES` (`src/lib/facturacionRoutes.ts`). Módulo sidenav **FACTURA**: Crear / Facturas / Presupuestos. **Crear** (`FacturaCrearPageClient`): cabecera en card — grid 3 cols (Fecha con picker nativo AR, Tipo De Factura select con labels MAYÚSCULAS, N° Comprobante solo lectura vacío) + Cliente en 2.ª fila. Segundo bloque (`FacturaCrearLineasBlock`): lupa (avanzada stub) + input; al foco/click abre **lista fija** (`h-72`); filtrado desde **3 letras** (máx. 10 ítems, `buscarProductosFacturaAction`); **click en fila = agregar** (sin botón `+`); filas minimalistas con hover `bg-accent/60`. Tabla remito COD. / DESCRIPCIÓN / CANTIDAD / PX. LISTA / TOTAL (estado local; mismo `cod_tienda` suma cantidad). Tipos: `FACTURA_TIPOS` / `FACTURA_TIPO_LABELS` en `@/lib/factura`. Facturas / Presupuestos: placeholders CFTL sin datos.
