...
- **Cobros por sucursal** (`/vtas-cobros/cobros-por-sucursal`): sidenav **VTAS. & COBROS → Cobros por sucursal**. Filas = `cobros_cx_fin` con `habilitado = true`. Columnas = sucursales con depósito. Cada celda = Select de caja destino (misma entidad; sin CHEQUE; `none` = SIN DESTINO). Tabla `cobros_por_sucursal`; Suc1/Suc2 pueden compartir caja. Editor guarda al cambiar el Select.
