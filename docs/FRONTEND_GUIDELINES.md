...
**Administración** (`AdministracionAccordionNav`): **FINANZAS** (TESORERIA → Fondos / Flujo De Fondos | BALANCE | OPERACIONES → COMPRAS / GASTOS | IMPUESTOS) → **LISTA PRECIOS** (PX TIENDA | PROVEEDORES | ANÁLISIS M.C.) → **VTAS. & COBROS** (Ptos. Vtas. / Cx. Fin. Cobros, pantallas directas) → **PEDIDO A FÁB.** → **ESTADÍSTICAS** → **USUARIOS**. Acordeón anidado: el grupo padre sigue abierto mientras un subgrupo hijo está expandido. **IMPUESTOS** agrupa Posición De IVA (`/finanzas/posicion-iva`).
...
- **Cx. Fin. Cobros** (`/vtas-cobros/cx-fin-cobros`, alias `/finanzas/analisis-mc/costos-financieros`): sidenav **VTAS. & COBROS → Cx. Fin. Cobros**. Header **VTAS. & COBROS** / Cx. Fin. Cobros. Matriz **FORMA DE PAGO × ENTIDAD** (`fin_ana_cos_fina`: `pago_id` → `cobros_forma_pago`, `terminal_id` → `tesoreria_cobros_entidades`). Columnas: **HAB.** | **FORMA DE PAGO** | **ENTIDAD** | **CUOTAS** (catálogo `cobros_cuotas`; UI placeholder hasta cablear FK) | **DÍAS DE ACREDITACIÓN** | **ARANCEL** | **CX FINANCIERO** | **IMP. CHEQUE** | **CX TOTAL S/ IVA** | **CX TOTAL C/ IVA**. Orden alfabético de filas: **FORMA DE PAGO** → **ENTIDAD** → **CUOTAS** (cuando exista). **GESTIONAR ENTIDADES** / **GESTIONAR FORMAS PAGO** / **GESTIONAR CUOTAS** / **Cálculo Cx. Total** (menú ACCIONES). Formas Pago: mismo patrón de catálogo que Entidades (§1.4); el agarre reordena columnas de Margen Contribución. Alta/edición de forma de pago: checkboxes de **entidades** (N:M `cobros_forma_pago_entidades`, mín. 1); la lista muestra las entidades asociadas.
