...
- **Panel:** `TYPEAHEAD_LISTBOX_PANEL_CLASS` + altura `TYPEAHEAD_LISTBOX_PANEL_HEIGHT_CLASS` (`h-72`). Ancho = input → `TYPEAHEAD_LISTBOX_PANEL_MATCH_INPUT_WIDTH_CLASS`; más ancho que el input → `TYPEAHEAD_LISTBOX_PANEL_WIDER_THAN_INPUT_CLASS` (clientes).
