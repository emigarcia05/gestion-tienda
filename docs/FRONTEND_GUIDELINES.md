...
- **Cx. Fin. Cobros** (`/vtas-cobros/cx-fin-cobros`, alias `/finanzas/analisis-mc/costos-financieros`): sidenav **VTAS. & COBROS → Cx. Fin. Cobros** (antes Costos Financieros / Cx. Financieros). Header **VTAS. & COBROS** / Cx. Fin. Cobros. `FinAnaCosFinaPageClient`; tabla HABILITADO | TERMINAL | **ID DUX** (`fin_ana_cos_fina_terminales.id_dux` = `id_terminal` DUX de cobros) | PAGO | … . Catálogo **Gestionar Terminales** (nombre + ID DUX) / pagos y **Cálculo Cx. Total**.
