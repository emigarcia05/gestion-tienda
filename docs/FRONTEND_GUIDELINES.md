...
- **Cobros & Cajas** (`/vtas-cobros/cobros-por-sucursal`): sidenav **VTAS. & COBROS → Cobros & Cajas**. Filtros `FilaFiltrosDesplegables` `columnas={4}`: **FORMA PAGO** | **ENTIDAD** | **VINCULADO** (`si`/`no` → SI / NO; SI = caja en todas las sucursales de la grilla) + contador + `LimpiarFiltrosButton`. Filas = combinaciones **forma de pago × entidad** con al menos una fila `habilitado` en `cobros_cx_fin` (todas las cuotas de esa combinación comparten destino). Columnas = sucursales con depósito. Cada celda: botón ícono `Link2` (sin caja) o `Pencil` (con caja) → modal **SELECCIONAR CAJA** con lista `TIPO CAJA - ENTIDAD - SUCURSAL - TITULAR` (misma entidad; sin CHEQUE) + **Quitar vínculo**. Tabla `cobros_por_sucursal` UNIQUE `(pago_id, entidad_id, sucursal_id)`. Editor guarda al elegir/quitar.
