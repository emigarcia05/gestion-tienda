...
- **Fact & Cobros** (`/finanzas/fact-cobros`): sidenav **FINANZAS → OPERACIONES → VENTAS**. Filtros MES / AÑO. Tabla **NRO. PTO. VTA.** | **MONTO GRAVADO** (totales persistidos). Header **Ptos. Vta.** (`GestionarGlobalPtoVtasModal`: catálogo `global_pto_vtas` + sucursales asociadas; alta/edición/baja solo editor). Header editor **Sincronizar** (GET DUX `/facturas` del mes; encadena POST `/api/sync-facturas-ventas-dux`). No lista facturas individuales.
