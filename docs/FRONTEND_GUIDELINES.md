...
SSOT `FACTURACION_ROUTES` (`src/lib/facturacionRoutes.ts`). Módulo sidenav **FACTURA**: Crear / Facturas / Presupuestos. **Crear** (`FacturaCrearPageClient`): cabecera en card — **una fila** `grid-cols-4` (Fecha, Tipo De Factura MAYÚSCULAS, N° Comprobante, Cliente). Segundo bloque (`FacturaCrearLineasBlock`): lupa (avanzada stub) + input; la **lista fija** (`h-72`, filas `py-0` + `divide-y divide-primary/40`, columnas centradas) solo se abre con **≥ 3 letras** (COD. / DESCRIPCIÓN / PRECIOS / STOCK / ícono sucursal `Store`); stock local `0` → `AlertTriangle` `text-destructive`; si no hay stock local pero sí en otra sucursal, `Store` en `text-primary`; click fila = agregar **siempre una fila nueva** (mismo COD. → otra línea; no suma cantidades; cierra + foco CANTIDAD con select; **Enter** en cantidad vuelve al buscador); ícono sucursal abre `FacturaProductoStockModal` (descripción + sucursal/stock). Stock de columna = sucursal del usuario; detalle = sucursales con depósito. Tabla remito en `.contenedor-tabla-gestion` (scroll vertical; **thead sticky**; columnas centradas; al agregar ítem nuevo scroll al final): basura | COD. | DESCRIPCIÓN | CANTIDAD | PX. LISTA | TOTAL (importes con `$` + `fmtPrecio`). Con descuento: columnas fijas **DESC.** (input % por ítem; `null` hereda el global del pie) + **PX C/ DESC.** + TOTAL. Pie: izquierda **zona descuentos** (`bg-primary/5` + `border-r-2 border-primary`, botón **Desc.** → `FacturaDescuentoModal` con DESC. % / DESC. TOTAL); derecha **resumen solo lectura** (`bg-muted/40` + `items-start`; sin desc. muestra `0%` / `$0`: TOTAL ITEM | TOTAL $ | DESC. % PROMEDIO ponderado | DESC. $ | TOTAL C/ DESC.). Al agregar ítem: con **DESC. %** hereda el global; con **DESC. TOTAL** congela el % promedio y lo hereda (`prepararDescuentoAlAgregarLinea`). Helpers en `@/lib/factura` (`FacturaDescuentoEstado`, `descuentoPctEspecial`, `resumenTotalesFactura`, …). Tipos: `FACTURA_TIPOS` / `FACTURA_TIPO_LABELS`. Facturas / Presupuestos: placeholders CFTL sin datos.
