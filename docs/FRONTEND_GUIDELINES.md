...
- **Fact & Cobros** (`/finanzas/fact-cobros`): sidenav **FINANZAS → OPERACIONES → VENTAS**. Filtros MES / AÑO. Tabla **PTO. VENTA** | **NOMBRE PTO. VENTA** | **TOTAL** (desde `global_pto_vtas` + `fin_fact_cobros_pto_vta_mes`). Header **Ptos. Vta.** (`GestionarGlobalPtoVtasModal`: catálogo `global_pto_vtas`; **SUC. ASOCIADAS** solo `global_sucursales.genera_est`; alta/edición/baja solo editor). Header editor **Sincronizar** (GET DUX `/v2/remitos-venta` del mes; suma `total_factura_asociada` solo si la factura asociada es **A** o **C** (`nro_factura_string`); encadena POST `/api/sync-facturas-ventas-dux`; solo `nro_pto_vta` del catálogo y sucursal asociada). No lista remitos individuales.
