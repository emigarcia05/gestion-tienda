...
- **Cobros & Cajas** (`/vtas-cobros/cobros-por-sucursal`): sidenav **VTAS. & COBROS → Cobros & Cajas**. Header editor **ACCIONES → Crear Cobro** (`ToolbarActionButton` + `Plus` → `CrearEditarCobroModal`: **FORMA DE PAGO** · **ENTIDAD** (solo N:M de la forma) · **CAJA VINCULADA** (`TIPO CAJA - ENTIDAD - SUCURSAL - TITULAR`; misma entidad; no CHEQUE; con sucursal operativa) · **OBSERVACIÓN** texto libre). Filtros `FilaFiltrosDesplegables` `columnas={4}`: **FORMA PAGO** | **ENTIDAD** + contador + `LimpiarFiltrosButton`. Tabla: **FORMA PAGO** | **ENTIDAD** | columnas = sucursales con depósito (celda = titular de la caja o `—`) | **ACCIONES** editor (lápiz + tacho). Filas = grupos manuales en `cobros_vinc_cajas` (no desde `habilitado` de Cx. Fin.). Una forma × entidad puede tener caja en una sola sucursal o en varias (editar eligiendo otra caja de otra SUC). Borrar elimina todo el grupo.
