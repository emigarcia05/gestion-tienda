...
- Urgente / enviar / tintométrico: `pedidos.ts` + `pedidosEnvio.service.ts`. Pedido Urgente: sucursal **y** un segundo filtro (`hayFiltroExtraPedidoUrgente`: PROVEEDOR, PEDIDO o `q` ≥ 3). `getListaPreciosParaPedidoUrgente` lista `prod_precios_provee.habilitado` de mercadería (`proveedorMercaderia` y `es_fabrica = false`). **Registrados en Dux** = vínculo `prod_precios_provee.cod_tienda` → fila `prod_tienda`; si no hay vínculo, **Sin Registrar**. `comprobarItemsParaGenerarPedidoAction` usa el **servicio** `getItemsTablaEnviarPedido` (no la Action vecina).
