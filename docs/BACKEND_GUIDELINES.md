...
**Búsqueda de productos (Crear):** Action `buscarProductosFacturaAction` → `buscarProductosParaFactura` (`facturaProductos.service.ts`). Zod `buscarProductosFacturaSchema` (`q` + `take` ≤ 10). Tokens de `q` separados por espacio: **AND** de `descripcion_tienda contains` (insensitive). Incluye `pxLista` de lista DUX **1 - GENERAL** (`encontrarIdListaGeneralPxListas`); si no hay precio → `0`. Asignación a usuarios: `modulos_permitidos` incluye `facturacion` (Zod max 4; CHECK SQL alineado en migración `20260912200000_global_personal_modulos_permitidos_facturacion`).
