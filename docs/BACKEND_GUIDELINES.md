...
**Borde:** `emitirFacturaComprobanteAction` / `emitirNotaCreditoFacturaAction` / `consultarFacturaComprobanteArcaAction` (`requireFacturacionLectura` + Zod + `revalidatePath` `/facturacion/factura/*`). El SOAP/PEM se carga con `import()` al emitir, no al abrir Crear. Lectura PDF: `obtenerFacturaComprobantePdfAction` (`requireFacturacionLectura`). Health: `GET /api/arca/health` (`guardFacturacionLectura`, `FEDummy`; no expone WSAA). Constancia: `GET /api/arca/constancia` (`guardClientesMutacion`; no expone WSAA). Listados RSC → `facturaComprobantesListado.service.ts` (`listarFacturasComprobantes` / `listarPresupuestosComprobantes` / `listarSucursalesFiltroFacturas` / `listarFacturaPtoVtasActivos` / `listarFacturasAutorizadasParaNc` / `listarCobrosComprobanteVta`; sin SOAP/PEM). Cada ítem incluye `sucursalCodigos` / `sucursalNombres` (vía `ptos_vtas` → `global_pto_vta_sucursales`) y `usuarioNombre` (`personal`). El filtro de sucursal de la lista es en cliente (default sucursal preferida; **TODAS** = sin filtro). Periodo (hoy / mes / todo / rango) y pendiente de pago (`saldoPendiente != null`) también se aplican en cliente sobre el lote (máx. 500). Lista Comprobantes: **SALDO PEND.** = `imp_total` − `imp_cobrado` en ventas no rechazadas sin NC si queda resto (aunque `dias_vencimiento` sea null, p. ej. CF); **DÍAS P/ VENC.** solo si hay plazo. Cobros de fila: `listarCobrosComprobanteFacturaAction`.
