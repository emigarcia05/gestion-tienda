...
Otras APIs: import lista, parse PDF, sync competencia, import/borrar `est_por_prod`, detalle historial pedidos, PDF comprobante de envíos (`GET /api/envios/[id]/comprobante`), sync remitos de venta Fact & Cobros (`POST /api/sync-facturas-ventas-dux`), health ARCA (`GET /api/arca/health`, `guardFacturacionLectura`, `FEDummy`), constancia ARCA (`GET /api/arca/constancia?cuit=`, `guardClientesMutacion`, `getPersona_v2`). Todas con guard en `apiRouteAuth` (o el mismo criterio).
...
| `POST /api/sync-cobros-dux`, `duxCobrosApi.ts`, `syncCobrosDuxRunStep` | Lectura `listarFinVtasCobros` sobre `fin_vtas_cobros` (sin import DUX) |
| `fin_ana_cos_fina_terminales`, `FinAnaCosFinaTerminal`, `GestionarTerminalesFinAnaCosFinaModal` | Marcas en `cx_fin_cobro_marcas`; listado cobros sin catálogo DUX |
