...
Servicios: `clientes.service.ts`, `enviosDirecciones.service.ts`, `enviosFinal.service.ts`. Actions: `src/actions/envios.ts` (incluye `marcarEnviosFinalEntregadoAction`; CRUD clientes con `requireClientesMutacion`). UI cliente: `CrearEditarClienteModal` (nombre, CONS. FINAL, CEL, **CUIT**, **CONDICIÓN IVA**, tipo, pintor, direcciones; compartido con Factura). El botón de sync del CUIT consulta `GET /api/arca/constancia` y precarga NOMBRE + CONDICIÓN IVA. Descarga PDF: `GET /api/envios/[id]/comprobante` (`guardEnviosLectura`). No borrar cliente/dirección si hay envío asociado ni cliente/proyecto si tiene `comprobantes_vtas` (`P2003` / Restrict). Alta/edición de dirección también desde el modal de cliente (`CONSUMIDOR_FINAL`): si el cliente no existe aún, se persiste antes de crear la dirección.
...
Módulo sidenav **COMPROBANTES**: Crear / Lista Comprobantes / Presupuestos. **CLIENTES**: Lista Clientes (`/facturacion/clientes/lista`). RSC `listarClientesConProyectos` (proyectos anidados de `clientes_proyectos`). Filtro de la lista en cliente: `matchByMultiTerm` con `numericAsContains` (CEL/CUIT por substring). En PINTOR, los asociados son `clientes` con `pintor_asociado` = id del pintor (`clientesAsociados`). Mutaciones: mismas actions de Envíos con `requireClientesMutacion`.
