...
**Contrato Zod** (`@/lib/validations/factura`): `facturaCrearCabeceraSchema`, `buscarProductosFacturaSchema`, `buscarClientesFacturaSchema`, `emitirFacturaComprobanteSchema` (líneas + descuento + receptor fiscal + `clienteId` opcional + `cbteAsocId` si NC), `facturaComprobanteIdSchema`. Alta cliente desde Factura: `crearClienteFacturaAction` → `crearCliente` / Zod `crearClienteSchema` (nombre obligatorio en este flujo; `cel` solo dígitos; `cuit` 11 o null; `condicion_iva` default CF). Typeahead clientes: `buscarClientesFacturaAction` → `buscarClientesParaFactura` (≥ 3 chars; tokens AND sobre nombre/cel/cuit). Constantes: `@/lib/factura`. Reglas A/B/C, CUIT DV, desglose IVA: `@/lib/facturaFiscal`.
