...
**Comprobante PDF (Crear, cliente):** `generarPdfFacturaComprobante` (`@/lib/generarPdfFacturaComprobante`) + `facturaComprobantePdfClient` (descarga `descargarPdfBytes`; imprimir = `window.open` + `print`). Nombre: `Cliente - dd-mm-aa - {últimos 4 dígitos N°} - (Observaciones).pdf` (sin observaciones se omite el paréntesis). Sin persistencia aún.
