...
- Urgente / enviar / tintométrico: `pedidos.ts` + `pedidosEnvio.service.ts`. Pedido Urgente con sucursal: `getListaPreciosParaPedidoUrgente` lista **todos** los `prod_precios_provee.habilitado` de mercadería (`proveedorMercaderia` y `es_fabrica = false`); orden **Registrados en Dux** (`prod_tienda` vía `cod_tienda`) y después sin registrar. `comprobarItemsParaGenerarPedidoAction` usa el **servicio** `getItemsTablaEnviarPedido` (no la Action vecina).
