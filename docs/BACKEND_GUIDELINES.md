...
**`ptos_vtas`** (ex `global_pto_vtas`): catálogo de puntos de venta. PK `id` cuid (TEXT; no serial — las FKs existentes dependen de ese id). `pto_venta` unique `CHAR(5)` con ceros a la izquierda (ej. `00002`; helper `normalizarPtoVentaCodigo`). Campos fiscales: `cuit` (11 dígitos o null), `ii_bb_multilateral` (bool), `ii_bb` (cuenta/convenio/`EXENTO`), `condicion_iva` (`Responsable Inscripto` \| `Monotributista` o null; CHECK), `nombre_titular` (mayúsculas `es-AR`), `domicilio_comercial`, `inicio_actividades` (`DATE`), `concepto` default `'1'`, `estado` default `'activo'` (`activo`\|`inactivo`; CHECK). **`suc_asociadas`:** N:M vía `global_pto_vta_sucursales` (`pto_vta_id` Cascade, `sucursal_id` Restrict a `global_sucursales` con `genera_est = true`; mín. 1 sucursal). CRUD editor: `globalPtoVtas.ts` + `globalPtoVtas.service.ts`. Lectura RSC en Ptos. Vtas. No borrar un pto. vta. si hay terminales `fin_ana_cos_fina_terminales.titular_id`.
