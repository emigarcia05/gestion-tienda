...
**Fact & Cobros** (`fin_fact_cobros_pto_vta_mes`): unique `(pto_vta_id, mes, anio, letra)` FK Restrict a `ptos_vtas`. No guarda remitos ni ítems: para Ptos. Vtas solo se persisten **totales mensuales por punto de venta + letra**. Sync editor: `POST /api/sync-facturas-ventas-dux` (pasos reanudables, 1 página DUX por POST) + `GET …/status`. Guard POST `guardFinanzasEditor`; status `guardFinanzasLectura`. Cliente DUX `duxRemitosVentaApi.ts` (`GET /WSERP/rest/services/v2/remitos-venta`, query `id_empresa` / `id_sucursal` / `fecha_desde` / `fecha_hasta` `yyyy-MM-dd`, `anulado=false`, limit 50; auth Bearer `DUX_API_TOKEN`; `id_empresa` = `DUX_ID_EMPRESA_COMPRAS`; `id_sucursal` = `id_dux` de sucursales asociadas al catálogo). Respuesta `datos` + `paginacion.hay_mas`. Regla: suma `total_factura_asociada` si no está anulado, estado `FACTURADO` (si viene), total > 0 y hay **letra de comprobante** asociada (A, B, C, …; `nro_factura_string` / `comprobantes_vinculados.nro_factura`; no usar `letra_comp` del remito). Un total por **punto de venta + letra**. Solo `nro_pto_vta` en `global_pto_vtas.pto_venta` y sucursal DUX en `suc_asociadas`. Periodo = `fecha` ISO. Listado RSC `listarFinFactCobrosPtoVtaMes` (consulta de totales; no hay pantalla de grilla). La UI `/vtas-cobros/ptos-venta` (alias `/finanzas/fact-cobros`) es el **catálogo** `ptos_vtas`.

**Cobros** (`fin_vtas_cobros`): una fila por línea de `cobranza` de GET `/v2/cobros` ([listar_cobros](https://developers.duxsoftware.com.ar/reference/listar_cobros)). Unique `(id_cobro, linea)` (`skipDuplicates`). Persistido: `id_cobro`, `id_sucursal` (= `global_sucursales.id_dux`, FK Restrict / Update Cascade; unique en `id_dux`, NULL permitido p. ej. corporativo), `fecha` (`@db.Date`, necesaria para la ventana), `descripcion`, `monto`, `id_tarjeta`, `id_plan_tarjeta`, `id_terminal`, `tipo_valor`. Cliente `duxCobrosApi.ts` (query `id_empresa` / `id_sucursal` / `fecha_desde` / `fecha_hasta`, limit 50). Sync editor: `POST /api/sync-cobros-dux` + `GET …/status` (`guardFinanzasEditor` / `guardFinanzasLectura`). Ventana: si no hay filas → día 1 del mes AR en curso; si hay → `max(fecha)` inclusive hasta hoy AR (reconsulta el último día y omite repetidos). Sucursales = `id_dux` asociadas al catálogo `ptos_vtas`; no se persiste cobro cuyo `id_sucursal` no esté en ese catálogo. Listado RSC `listarFinVtasCobros` por mes/año de `fecha` (join sucursal). UI `/vtas-cobros/cobros` (header **Gestionar Terminales**).

**`ptos_vtas`** (ex `global_pto_vtas`): catálogo de puntos de venta. PK `id` cuid (TEXT; no serial — las FKs existentes dependen de ese id). `pto_venta` unique `CHAR(5)` con ceros a la izquierda (ej. `00002`; helper `normalizarPtoVentaCodigo`). Campos fiscales (CRUD editor en `/vtas-cobros/ptos-venta` + `GestionarGlobalPtoVtasModal` / Zod `globalPtoVtas.ts`): `cuit` (11 dígitos o null), `ii_bb_multilateral` (bool), `ii_bb` (cuenta/convenio/`EXENTO`), `condicion_iva` INT nullable = **`pto_ventas_cod_arca.codigo`** (FK Restrict / Update Cascade; UI Select del catálogo ARCA; alta exige `activo`; edición puede conservar un código ya persistido aunque quede inactivo), `nombre_titular` (mayúsculas `es-AR`), `domicilio_comercial`, `inicio_actividades` (`DATE` ↔ ISO `YYYY-MM-DD` vía `isoYmdFromPrismaDateOnly`), `concepto` default `'1'`, `estado` default `'activo'` (`activo`\|`inactivo`; CHECK). **`suc_asociadas`:** N:M vía `global_pto_vta_sucursales` (`pto_vta_id` Cascade, `sucursal_id` Restrict a `global_sucursales` con `genera_est = true`; mín. 1 sucursal). **Reglas de elección** (consulta UI; aún no sustituyen el Select de Crear Factura): SSOT `@/lib/ptoVtaReglas` (`PTO_VTA_REGLAS`). 1) el pto. vta. debe estar en `suc_asociadas` de la sucursal que emite; 2) factura fiscal + cliente RI (`condicion_iva` = `ARCA_CONDICION_IVA.RI` / `PTO_VTA_REGLA_RI_CODIGO`) → pto. vta. con la misma cond. IVA. Listado UI: N° / titular / sucursales / CUIT / IIBB / CONV. MULT. SI/NO / domicilio / inicio. Sync DUX de totales (`fin_fact_cobros_pto_vta_mes`) queda en `POST /api/sync-facturas-ventas-dux` (no en el header de Ptos. Vtas.). CRUD editor: `globalPtoVtas.ts` + `globalPtoVtas.service.ts`. Lectura RSC en Ptos. Vtas. No borrar un pto. vta. si hay terminales `fin_ana_cos_fina_terminales.titular_id`.
...
**Análisis M.C.:** `fin_ana_cos_fina` (matriz marca × pago: `terminal_id` → `fin_ana_cos_fina_terminales_marcas`; no apunta a la terminal DUX) + fórmulas `fin_ana_mc_formulas` + categorías `fin_ana_mc_cat`. Signo descuento: `1 + %/100` (negativo = descuento). UI categorías: `reemplazarFinAnaMcCategoriasAction` (no CRUD granular). **Margen Contribución** UI `/finanzas/analisis-mc/margen-contribucion` (filtro TERMINAL = marca). **Cx. Fin. Cobros** UI canónica `/vtas-cobros/cx-fin-cobros` (alias `/finanzas/analisis-mc/costos-financieros`). **Marcas** `fin_ana_cos_fina_terminales_marcas` (ex `fin_ana_cos_fina_terminales`; `nombre` unique; Prisma `FinAnaCosFinaTerminalMarca`). **Terminales DUX** `fin_ana_cos_fina_terminales` (Prisma `FinAnaCosFinaTerminal`): `id_dux` unique (carga el usuario; = `id_terminal` de cobros), `marca_id` Restrict, `titular_id` Restrict a `ptos_vtas`. UI de catálogo en **Cobros** (`/vtas-cobros/cobros`, `GestionarTerminalesFinAnaCosFinaModal`). Actions `finAnaCosFina.ts`: marcas/terminales revalidan Cobros, la ruta canónica de Cx. Fin., el alias y Margen Contribución.
...
Otras APIs: import lista, parse PDF, sync competencia, import/borrar `est_por_prod`, detalle historial pedidos, PDF comprobante de envíos (`GET /api/envios/[id]/comprobante`), sync remitos de venta Fact & Cobros (`POST /api/sync-facturas-ventas-dux`), sync cobros DUX (`POST /api/sync-cobros-dux`), health ARCA (`GET /api/arca/health`, `guardFacturacionLectura`, `FEDummy`), constancia ARCA (`GET /api/arca/constancia?cuit=`, `guardClientesMutacion`, `getPersona_v2`). Todas con guard en `apiRouteAuth` (o el mismo criterio).
