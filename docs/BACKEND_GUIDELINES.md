...
Rename tablas: migración `20260915210000_rename_comprobantes_vtas`. Valores previos: `comprobante`→`factura_no_fiscal`, `factura`→`factura_fiscal`, `nota_credito`→`nota_credito_fiscal`. Cliente vacío o `CONSUMIDOR FINAL` sin selección de lista → `CONSUMIDOR FINAL` (fiscal: DocTipo 99 / DocNro 0 / cond. IVA 5). Cliente con `cliente_id`: `receptorFiscalDesdeCliente` usa CUIT válido + `condicion_iva`; si falta alguno → CF. La UI de Crear no envía COND. IVA / TIPO DOC. / NRO. DOC. Otro texto sin `cliente_id` del typeahead → no emite.
...
**PDF:** `generarPdfFacturaComprobante` + `facturaComprobantePdfClient`. Si hay CAE, el PDF lo imprime con vencimiento. Nombre: `Cliente - dd-mm-aa - {últimos 4 dígitos N°} - (Comentarios).pdf`. Modal post-emisión: catálogo de cobro (`listarCatalogoCobroFacturaAction`, `requireFacturacionLectura`) lee `cobros_forma_pago` + `cobros_cuotas`. Al cerrar o imprimir se persisten líneas en `comprobantes_vtas_cobros` + `imp_cobrado` (suma no CC) y `dias_vencimiento` (plazo CC o null). Aún **no** escribe `tesoreria_movimientos`. Si `entidad_obligatoria` es false, el cobro no pide entidad (`entidadNombre` puede ser `""`). **CUENTA CORRIENTE** es sentinel de UI (`FACTURA_FORMA_PAGO_CUENTA_CORRIENTE`), no una fila de `cobros_forma_pago`: cubre el saldo pendiente (impago o parcial). El **PLAZO** (1–365) sale de `clientes.cta_corriente_plazo` o lo carga el usuario; sin plazo no se cierra ni imprime. **Tope CC:** `clientes.cta_corriente_monto_max`; si el saldo CC ya lo supera, `emitirFacturaComprobante` rechaza ventas fiscales y no fiscales. **`dias_vencimiento`:** INT nullable 1–365 en `comprobantes_vtas`. Persistencia `guardarDiasVencimientoFacturaAction` → `guardarDiasVencimientoComprobante`. El recordatorio a X días aún no está implementado. Emitir exige `personalId` del usuario de pestaña (`MENSAJE_PERSONAL_SESION_REQUERIDO`). Migraciones extra: `20260922180000_rename_global_personal_to_personal` + `20260922190000_comprobantes_vtas_imp_cobrado` (ya en Neon) + `20260923120000_comprobantes_vtas_cobros` (**nueva**; aplicar en Neon al deploy).
