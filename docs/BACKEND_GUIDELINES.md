...
<<<<<<< HEAD
Módulos sidenav **FACTURA**: Crear / Facturas / Presupuestos. **CLIENTES**: Lista Clientes (`/facturacion/clientes/lista`). RSC `listarClientesConProyectos` (proyectos anidados de `clientes_proyectos`). Mutaciones: mismas actions de Envíos (`crearClienteAction` / `editarClienteAction` / `eliminarClienteAction` / CRUD `enviosDireccion*`) con `requireClientesMutacion`.
=======
