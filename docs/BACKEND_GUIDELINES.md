...
**Análisis M.C.:** `fin_ana_cos_fina` (matriz entidad × pago: `terminal_id` → `cobros_entidades`) + fórmulas `fin_ana_mc_formulas` + categorías `fin_ana_mc_cat`. Signo descuento: `1 + %/100` (negativo = descuento). UI categorías: `reemplazarFinAnaMcCategoriasAction` (no CRUD granular). **Margen Contribución** UI `/finanzas/analisis-mc/margen-contribucion` (filtro TERMINAL = entidad/`cobros_entidades`). **Cx. Fin. Cobros** UI canónica `/vtas-cobros/cx-fin-cobros` (alias `/finanzas/analisis-mc/costos-financieros`). **Entidades** (UI; código histórico “marcas”) `cobros_entidades` (ex `cobros_terminales` / `cobros_terminal_marca` / `cobros_cx_fin_marcas` / `cx_fin_cobro_marcas` / `fin_ana_cos_fina_terminales_marcas`; `nombre` unique; Prisma `FinAnaCosFinaTerminalMarca`). **Formas de pago** `cobros_forma_pago` (ex `cobros_opciones_pago` / `fin_ana_cos_fina_pagos`; Prisma `FinAnaCosFinaPagoCat`; `nombre` unique; **sin** `codigo`; bools `asociado_terminal` / `asociado_banco`). **Cuotas** `cobros_cuotas` (Prisma `CobrosCuota`; `cantidad` unique 1–99; seed 1/3/6/9/12/18; CRUD `cobrosCuotas.service.ts` + actions en `finAnaCosFina.ts`; aún **sin** FK en `fin_ana_cos_fina`). **Sin** tabla `fin_ana_cos_fina_terminales` ni UI de terminales DUX. Actions `finAnaCosFina.ts`: entidades revalidan la ruta canónica de Cx. Fin., el alias y Margen Contribución.
...
| `fin_ana_cos_fina_terminales`, `FinAnaCosFinaTerminal`, `GestionarTerminalesFinAnaCosFinaModal` | Marcas en `cobros_entidades` |
| `cx_fin_cobro_marcas`, `cobros_cx_fin_marcas`, `cobros_terminal_marca` | `cobros_entidades` |
...
| `fin_tesoreria_entidades` | `tesoreria_entidades` |
| `fin_tesoreria_cheques` | `tesoreria_cheques` |
| `cobros_bancos`, `CobrosBanco` | Eliminado |
| `cobros_terminales` | `cobros_entidades` |
