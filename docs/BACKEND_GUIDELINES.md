...
**Comprobante PDF (Crear, cliente):** `generarPdfFacturaComprobante` (`@/lib/generarPdfFacturaComprobante`) + `facturaComprobantePdfClient` (descarga `descargarPdfBytes`; imprimir = `window.open` + `print`). Nombre: `Cliente - dd-mm-aa - N° Comprobante.pdf`. Sin persistencia aún.
