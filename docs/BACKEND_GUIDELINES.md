...
Módulo sidenav **COMPROBANTES**: Crear / Lista Comprobantes / Presupuestos. **CLIENTES**: Lista Clientes (`/facturacion/clientes/lista`). RSC `listarClientesConProyectos` (proyectos anidados de `clientes_proyectos`; incluye `cta_corriente_plazo` y `cta_corriente_monto_max`). Filtro de la lista en cliente: `matchByMultiTerm` con `numericAsContains` (CEL/CUIT por substring). Si `es_pintor`, los asociados son `clientes` con `pintor_asociado` = id del pintor (`clientesAsociados`). Mutaciones: mismas actions de Envíos (`crearClienteAction` / `editarClienteAction` / `eliminarClienteAction` / CRUD `enviosDireccion*`) con `requireClientesMutacion`.
