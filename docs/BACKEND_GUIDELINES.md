...
**Tesorería:** `CajaTesoreria.tipoCaja` usa enum `TipoCajaTesoreria` (valor `TARJETAS_A_COBRAR` se muestra **TERMINAL DE PAGO**). **`tipo_valor`:** enum `TipoValorTesoreria` (DIGITAL / EFECTIVO / CHEQUE); se deriva de `tipo_caja` en alta (`tipoValorDesdeTipoCaja`). **Sin** columna `disponibilidad` (eliminada). **`sucursal_id`:** FK Restrict a `sucursales`; **null solo si `tipo_caja = CHEQUE`** (CHECK `tesoreria_cajas_sucursal_cheque_chk`; Zod `superRefine` + `resolverSucursalCajaTesoreria` + helper UI `cajaTesoreriaUsaSucursal`). Unique **solo en SQL** (Prisma no lleva `@@unique` porque son índices parciales): entidad+titular+sucursal si hay sucursal; entidad+titular si CHEQUE. **`titular`:** nombre de `global_personal` con `titular_financiero = true` (`listNombresTitularesFinancieros` / `resolverNombreTitularFinanciero`). **`entidad_id`:** FK a `tesoreria_cobros_entidades` (catálogo unificado con Cx. Fin. Cobros; Prisma `FinAnaCosFinaTerminalMarca`). CRUD de entidades desde Cajas y Cx. Fin. revalida `/finanzas/tesoreria` + rutas de análisis MC. **`supervision_fiscal`:** bool NOT NULL default `false` (Supervisión Fiscal). **Baja de caja** (`eliminarCajaTesoreria` / `eliminarCajaTesoreriaAction`): editor; bloqueada si hay `tesoreria_cheques.caja_id`; al borrar, `caja_destino_id` de cheques acreditados a esa caja queda null (`onDelete: SetNull`). UI: tacho en ACCIONES de Cajas → `EliminarCajaTesoreriaModal`. **Catálogo `tesoreria_titulares`:** Prisma `TesoreriaTitular`; PK cuid automático; `nombre` unique (MAYÚSCULAS `es-AR`, lo completa el usuario). Seed de migración: DISTINCT de `tesoreria_cajas.titular`, `tesoreria_cheques.tenedor` y `global_personal.nombre_personal` con `titular_financiero = true`. CRUD: `tesoreriaTitulares.service.ts` + Actions `tesoreriaTitulares.ts` (`requireFinanzasLectura` / `requireEditorFinanzas`). Baja bloqueada si el nombre figura en `ptos_vtas.titular` (FK Restrict), `tesoreria_cajas.titular` o `tesoreria_cheques.tenedor`. UI editor: **Gestionar Titulares** en Cajas y Ptos. Vtas. **Aún no** sustituye el TEXT de `tesoreria_cajas.titular` / `tesoreria_cheques.tenedor`. Catálogo `tesoreria_tipo_caja` (`FinTesoreriaTipoCaja`): CRUD UI **Gestionar Tipo Caja** en Cajas (`tesoreriaTipoCaja.ts` / `GestionarTesoreriaTipoCajaModal`). Cheques: `finTesoreriaCheques.ts` (tenedor = misma fuente de personal).
...
**Análisis M.C.:** `fin_ana_cos_fina` (matriz entidad × pago: `terminal_id` → `tesoreria_cobros_entidades`) + fórmulas `fin_ana_mc_formulas` + categorías `fin_ana_mc_cat`. Signo descuento: `1 + %/100` (negativo = descuento). UI categorías: `reemplazarFinAnaMcCategoriasAction` (no CRUD granular). **Margen Contribución** UI `/finanzas/analisis-mc/margen-contribucion` (filtro TERMINAL = entidad/`tesoreria_cobros_entidades`). **Cx. Fin. Cobros** UI canónica `/vtas-cobros/cx-fin-cobros` (alias `/finanzas/analisis-mc/costos-financieros`). Listado `listarFinAnaCosFina`: **una fila por vínculo** de `cobros_forma_pago_entidades` (formas con `en_costos_financieros`); `ensureFinAnaCosFinaSeed` crea faltantes y borra huérfanas (ya no producto cartesiano marca×pago). Orden alfabético `es` por forma de pago → entidad (luego cuotas al cablear FK). **Entidades** (UI unificada Cobros + Cajas) `tesoreria_cobros_entidades` (ex `cobros_entidades` / `tesoreria_entidades` / `cobros_terminales` / …; Prisma `FinAnaCosFinaTerminalMarca`; `nombre` unique; `orden`). `tesoreria_cajas.entidad_id` FK a esta tabla. **Formas de pago** `cobros_forma_pago` (ex `cobros_opciones_pago` / `fin_ana_cos_fina_pagos`; Prisma `FinAnaCosFinaPagoCat`; `nombre` unique; **sin** `codigo`; bools `asociado_terminal` / `asociado_banco`). **N:M** `cobros_forma_pago_entidades` (Prisma `CobrosFormaPagoEntidad`; PK `(pago_id, entidad_id)`; Cascade pago / Restrict entidad): cada forma de pago **exige ≥ 1 entidad** (Zod + servicio). La elección se hace solo en **Gestionar Formas Pago** (alta/edición). Sync de filas `fin_ana_cos_fina` al crear/editar vínculos y al listar. Baja de entidad bloqueada si es la única de algún pago. **Cuotas** `cobros_cuotas` (Prisma `CobrosCuota`; `cantidad` unique 1–99; seed 1/3/6/9/12/18; CRUD `cobrosCuotas.service.ts` + actions en `finAnaCosFina.ts`; aún **sin** FK en `fin_ana_cos_fina`). **Sin** tabla `fin_ana_cos_fina_terminales` ni UI de terminales DUX. Actions `finAnaCosFina.ts`: entidades revalidan Cx. Fin., alias, Margen Contribución y `/finanzas/tesoreria`.
...
| `fin_tesoreria` | `tesoreria_cajas` (Prisma `CajaTesoreria`)
