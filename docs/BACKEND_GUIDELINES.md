...
**Fact & Cobros** (`fin_fact_cobros_pto_vta_mes` + `fin_fact_cobros_items`): agregado mensual unique `(pto_vta_id, mes, anio, letra)` FK Restrict a `global_pto_vtas`; además se persiste cada ítem de cobro (remito contabilizado) en `fin_fact_cobros_items` para cálculos/estadísticas. Sync editor: `POST /api/sync-facturas-ventas-dux` (pasos reanudables, 1 página DUX por POST) + `GET …/status`. Guard POST `guardFinanzasEditor`; status `guardFinanzasLectura`. Cliente DUX `duxRemitosVentaApi.ts` (`GET /WSERP/rest/services/v2/remitos-venta`, query `id_empresa` / `id_sucursal` / `fecha_desde` / `fecha_hasta` `yyyy-MM-dd`, `anulado=false`, limit 50; auth Bearer `DUX_API_TOKEN`; `id_empresa` = `DUX_ID_EMPRESA_COMPRAS`; `id_sucursal` = `id_dux` de sucursales asociadas al catálogo). Respuesta `datos` + `paginacion.hay_mas`. Regla: contabilizar/persistir ítems cuando no está anulado, estado `FACTURADO` (si viene), total > 0 y hay **letra de comprobante** asociada (A, B, C, …; `nro_factura_string` / `comprobantes_vinculados.nro_factura`; no usar `letra_comp` del remito). Un total por **punto de venta + letra** en el agregado, más un registro por remito válido en `fin_fact_cobros_items` (incluye `id_sucursal_dux`, `id_remito_venta`, `fecha_ymd`, `estado_facturacion`, `nro_factura_string`, `monto_gravado`). Solo `nro_pto_vta` en `global_pto_vtas.pto_venta` y sucursal DUX en `suc_asociadas`. Periodo = `fecha` ISO. Listado RSC `listarFinFactCobrosPtoVtaMes` (PTO. VTAS. `Nº - NOMBRE` / columnas por letra / TOTAL). UI canónica `/vtas-cobros/ptos-venta` (alias `/finanzas/fact-cobros`). Cobros: `/vtas-cobros/cobros`.
...
Otras APIs: import lista, parse PDF, sync competencia, import/borrar `est_por_prod`, detalle historial pedidos, PDF comprobante de envíos (`GET /api/envios/[id]/comprobante`), sync remitos de venta Fact & Cobros (`POST /api/sync-facturas-ventas-dux`). Todas con guard en `apiRouteAuth` (o el mismo criterio).
