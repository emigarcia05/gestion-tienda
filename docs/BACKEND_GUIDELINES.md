...
**Descuento (Crear, estado local):** `FacturaDescuentoEstado` en `@/lib/factura` — fuente `porcentaje` (mismo % a todos los PX. LISTA) o `total_fac` (objetivo de TOTAL C/ DESC.; el % se recalcula si cambian líneas/cantidades). Helpers: `porcentajeDescuentoEfectivo`, `pxConDescuento`, `resumenTotalesFactura`, `descuentoPctDesdeTotalFac`. Sin persistencia de comprobante aún. Asignación a usuarios: `modulos_permitidos` incluye `facturacion` (Zod max 4; CHECK SQL alineado en migración `20260912200000_global_personal_modulos_permitidos_facturacion`).
