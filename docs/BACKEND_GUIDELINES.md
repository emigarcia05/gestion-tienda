...
Rename tablas: migración `20260915210000_rename_comprobantes_vtas`. Valores previos: `comprobante`→`factura_no_fiscal`, `factura`→`factura_fiscal`, `nota_credito`→`nota_credito_fiscal`. Cliente `CONSUMIDOR FINAL` explícito sin selección de lista → CF (fiscal: DocTipo 99 / DocNro 0 / cond. IVA 5). Vacío no emite. Cliente con `cliente_id`: `receptorFiscalDesdeCliente` usa CUIT válido + `condicion_iva`; si falta alguno → CF. La UI de Crear no envía COND. IVA / TIPO DOC. / NRO. DOC. Otro texto sin `cliente_id` del typeahead → no emite.
...
**Flujo emitir (servicio `facturaComprobantes.service.ts`):** validar `personal_id` (fila en `personal`) → emisor activo (`ptos_vtas` + CUIT + cond. IVA; CUIT alineado a certificado / `ARCA_CUIT`) → PEM de `ARCA_CERT_PEM_{CUIT}` → receptor (`receptorFiscalParaEmitir`: catálogo del cliente o CF) → resolver `proyecto_id` (`resolverProyectoIdComprobante`) → armar `FECAERequest` → persistir borrador + ítems (`comprobantes_vtas_items`) + `personal_id` → `FECompUltimoAutorizado` + 1 → `FECAESolicitar` → guardar CAE / rechazo. Idempotencia: si ya hay CAE no reenviar; timeout de red → `FECompConsultar` del mismo nro. Historial en `comprobantes_vtas_historial_arca` (`request_id`, errores JSON/texto, **sin XML completo**).

**Borde:** `emitirFacturaComprobanteAction` / `emitirNotaCreditoFacturaAction` / `consultarFacturaComprobanteArcaAction` (`requireFacturacionLectura` + Zod + `revalidatePath` `/facturacion/factura/*`). Zod de emitir y NC exige `personalId` (`idPersonalSchema`). El SOAP/PEM se carga con `import()` al emitir, no al abrir Crear. Lectura PDF: `obtenerFacturaComprobantePdfAction` (`requireFacturacionLectura`). Health: `GET /api/arca/health` (`guardFacturacionLectura`, `FEDummy`; no expone WSAA). Constancia: `GET /api/arca/constancia` (`guardClientesMutacion`; no expone WSAA). Listados RSC → `facturaComprobantesListado.service.ts` (`listarFacturasComprobantes` / `listarPresupuestosComprobantes` / `listarSucursalesFiltroFacturas` / `listarFacturaPtoVtasActivos` / `listarFacturasAutorizadasParaNc`; sin SOAP/PEM). Cada ítem incluye `sucursalCodigos` (códigos de `sucursales` vía `ptos_vtas` → `global_pto_vta_sucursales`), `createdAtIso` (`created_at`; hora de emisión) y `personalId` / `usuarioNombre` (`personal` que generó; null en filas anteriores a `20260922180000`). El filtro USUARIO lee `listarUsuariosFiltroFacturas`. El filtro de sucursal de la lista es en cliente (default **TODAS**; sucursal preferida no se aplica sola). Periodo (hoy / mes / todo / rango) y pendiente de pago (`saldoPendiente != null`) también se aplican en cliente sobre el lote (máx. 500). Lista Comprobantes expone **SALDO PEND.** / **DÍAS P/ VENC.** si la venta tiene `dias_vencimiento`, no está rechazada y no tiene NC: saldo = `imp_total` (los cobros del modal aún no persisten); días = vencimiento (`fecha` + plazo) − hoy AR.
...
**PDF:** `generarPdfFacturaComprobante` + `facturaComprobantePdfClient`. Si hay CAE, el PDF lo imprime con vencimiento. Nombre: `Cliente - dd-mm-aa - {últimos 4 dígitos N°} - (Comentarios).pdf`. Modal post-emisión: catálogo de cobro (`listarCatalogoCobroFacturaAction`, `requireFacturacionLectura`) lee `cobros_forma_pago` + `cobros_cuotas`; la UI registra pagos parciales **en memoria** (aún no escribe `tesoreria_movimientos`). Si `entidad_obligatoria` es false, el cobro no pide entidad (`entidadNombre` puede ser `""`). **No** hay forma de pago Cuenta Corriente: el saldo pendiente (impago o parcial) no exige un cobro. Al emitir una venta con `cliente_id`, `emitirFacturaComprobante` escribe `comprobantes_vtas.dias_vencimiento` = `clientes.cta_corriente_plazo` (default 1; helper `diasVencimientoDesdePlazoCliente`). CONSUMIDOR FINAL / sin catálogo → `dias_vencimiento` null. **X / Cancelar** cierran sin persistir cobros (el vencimiento ya quedó al emitir). Imprimir / Descargar / Imprimir & Descargar: si el saldo del modal quedó en 0, `guardarDiasVencimientoFacturaAction` pone `dias_vencimiento` null; si queda pendiente, confirman el plazo del cliente; luego la UI navega a Lista Comprobantes o Presupuestos. **Tope CC:** `clientes.cta_corriente_monto_max`; si el saldo CC ya lo supera, `emitirFacturaComprobante` rechaza ventas fiscales y no fiscales. **`dias_vencimiento`:** INT nullable 1–365 en `comprobantes_vtas`. Persistencia `guardarDiasVencimientoFacturaAction` → `guardarDiasVencimientoComprobante`. El recordatorio a X días aún no está implementado.
