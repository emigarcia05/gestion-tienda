...
Servicios: `clientes.service.ts` (`listarClientes`, `listarClientesConProyectos`), `enviosDirecciones.service.ts`, `enviosFinal.service.ts`. Actions: `src/actions/envios.ts` (incluye `marcarEnviosFinalEntregadoAction`; CRUD clientes y `clientes_proyectos` con `requireClientesMutacion`; `revalidatePath` incluye `FACTURACION_ROUTES.clientes.lista`). UI cliente: `CrearEditarClienteModal` (nombre, CONS. FINAL, CEL, **CUIT**, **CONDICIÓN IVA**, **PLAZO CUENTA CORRIENTE** opcional, **MONTO MÁX. CTA. CTE.** opcional, tipo, pintor, **PROYECTOS**; compartido con Factura). El botón de sync del CUIT consulta `GET /api/arca/constancia` y precarga NOMBRE + CONDICIÓN IVA. Descarga PDF: `GET /api/envios/[id]/comprobante` (`guardEnviosLectura`). No borrar cliente/proyecto si hay envío asociado ni cliente si tiene `comprobantes_vtas` (`P2003` / Restrict). Alta/edición de proyecto también desde el modal de cliente (`CONSUMIDOR_FINAL`): si el cliente no existe aún, se persiste antes de crear el proyecto. Zod `crearEnviosDireccionSchema` incluye `nombreProyecto`.
