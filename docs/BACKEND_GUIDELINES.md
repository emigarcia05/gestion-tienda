...
<<<<<<< HEAD
**Análisis M.C.:** `fin_ana_cos_fina` (matriz marca × pago: `terminal_id` → `cobros_cx_fin_marcas`) + fórmulas `fin_ana_mc_formulas` + categorías `fin_ana_mc_cat`. Signo descuento: `1 + %/100` (negativo = descuento). UI categorías: `reemplazarFinAnaMcCategoriasAction` (no CRUD granular). **Margen Contribución** UI `/finanzas/analisis-mc/margen-contribucion` (filtro TERMINAL = marca). **Cx. Fin. Cobros** UI canónica `/vtas-cobros/cx-fin-cobros` (alias `/finanzas/analisis-mc/costos-financieros`). **Marcas** `cobros_cx_fin_marcas` (ex `cx_fin_cobro_marcas` / `fin_ana_cos_fina_terminales_marcas`; `nombre` unique; Prisma `FinAnaCosFinaTerminalMarca`). **Formas de pago** `cobros_forma_pago` (ex `fin_ana_cos_fina_pagos`; Prisma `FinAnaCosFinaPagoCat`; `nombre` unique; **sin** `codigo`; `asociado_terminal` / `asociado_banco` bool). **Bancos** `cobros_bancos` (Prisma `CobrosBanco`; `nombre` unique MAYÚSCULAS; `orden`; servicio `cobrosBancos.service.ts`; CRUD en `finAnaCosFina.ts`). **Sin** tabla `fin_ana_cos_fina_terminales` ni UI de terminales DUX. Actions `finAnaCosFina.ts`: marcas, pagos y bancos revalidan la ruta canónica de Cx. Fin., el alias y Margen Contribución.
=======
...
Servicios: `clientes.service.ts` (`listarClientes`, `listarClientesConProyectos`), `enviosDirecciones.service.ts`, `enviosFinal.service.ts`. Actions: `src/actions/envios.ts` (incluye `marcarEnviosFinalEntregadoAction`; CRUD clientes y `clientes_proyectos` con `requireClientesMutacion`; `revalidatePath` incluye `FACTURACION_ROUTES.clientes.lista`). UI cliente: `CrearEditarClienteModal` (nombre, CONS. FINAL, CEL, **CUIT**, **CONDICIÓN IVA**, tipo, pintor, **PROYECTOS**; compartido con Factura). El botón de sync del CUIT consulta `GET /api/arca/constancia` y precarga NOMBRE + CONDICIÓN IVA. Descarga PDF: `GET /api/envios/[id]/comprobante` (`guardEnviosLectura`). No borrar cliente/proyecto si hay envío asociado ni cliente si tiene `comprobantes_vtas` (`P2003` / Restrict). Alta/edición de proyecto también desde el modal de cliente (`CONSUMIDOR_FINAL`): si el cliente no existe aún, se persiste antes de crear el proyecto. Zod `crearEnviosDireccionSchema` incluye `nombreProyecto`.
...
Módulo sidenav **COMPROBANTES**: Crear / Lista Comprobantes / Presupuestos. **CLIENTES**: Lista Clientes (`/facturacion/clientes/lista`). RSC `listarClientesConProyectos` (proyectos anidados de `clientes_proyectos`). Filtro de la lista en cliente: `matchByMultiTerm` con `numericAsContains` (CEL/CUIT por substring). En PINTOR, los asociados son `clientes` con `pintor_asociado` = id del pintor (`clientesAsociados`). Mutaciones: mismas actions de Envíos (`crearClienteAction` / `editarClienteAction` / `eliminarClienteAction` / CRUD `enviosDireccion*`) con `requireClientesMutacion`.
...
<<<<<<< HEAD
| `fin_ana_cos_fina_terminales`, `FinAnaCosFinaTerminal`, `GestionarTerminalesFinAnaCosFinaModal` | Marcas en `cobros_cx_fin_marcas` |
| `cx_fin_cobro_marcas` | `cobros_cx_fin_marcas` |
| `fin_ana_cos_fina_pagos`, columna `codigo` | `cobros_forma_pago` (`asociado_terminal` / `asociado_banco`) |
=======
...
| `cobros_bancos`, `CobrosBanco` | Eliminado |
