...
Rename tablas: migración `20260915210000_rename_comprobantes_vtas`. Valores previos: `comprobante`→`factura_no_fiscal`, `factura`→`factura_fiscal`, `nota_credito`→`nota_credito_fiscal`. Cliente vacío o `CONSUMIDOR FINAL` sin selección de lista → `CONSUMIDOR FINAL` (fiscal: DocTipo 99 / DocNro 0 / cond. IVA 5). Otro texto sin `cliente_id` del typeahead → no emite.
